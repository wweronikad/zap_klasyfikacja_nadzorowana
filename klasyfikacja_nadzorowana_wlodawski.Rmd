...
par(mar = c(0, 0, 0, 0))
plot.new()
legend("center", legend = class_labels_rf, fill = colors_rf, title = "Klasy", bty = "n", ncol = 5, cex = 0.8)

# Legenda BDOT
par(mar = c(0, 0, 0, 0))
plot.new()
legend("center", legend = class_labels_bdot, fill = colors_bdot, title = "Klasy BDOT", bty = "n", ncol = 4, cex = 0.8)
...
# Layout dla dwóch map i jednej, wspólnej legendy po prawej
layout(matrix(c(1, 2, 3), nrow = 1), widths = c(0.425, 0.425, 0.15))

# Mapa 1: Klasyfikacja RF
par(mar = c(2, 2, 3, 1))
...
Dodanie kanału SWIR i indeksów spektralnych: Do analizy zostanie włączony dodatkowy kanał SWIR (Short-Wave Infrared) oraz obliczone na jego podstawie indeksy spektralne: NDVI, NDWI i NDBI. Pozwoli to na lepsze różnicowanie typów pokrycia terenu.
...
# Pobieranie kanału SWIR dla wybranych kafli
swir_channel <- "swir16"

print("Sprawdzanie i pobieranie kanału SWIR...")

for (tile in tile_ids_2023) {
  tile_dir <- file.path(base_dir_2023, tile)
  
  if (!dir.exists(tile_dir)) {
    warning(paste("Katalog dla kafla", tile, "nie istnieje. Pomijanie pobierania SWIR."))
    next
  }
  
  # Pobieranie URL dla kanału SWIR
  swir_url <- obrazy_2023 |>
    items_filter(properties$`s2:tile_id` == tile) |>
    assets_select(asset_names = swir_channel) |>
    assets_url()
    
  if (length(swir_url) > 0) {
    file_path <- file.path(tile_dir, basename(swir_url))
    if (!file.exists(file_path)) {
      print(paste("Pobieranie:", basename(file_path)))
      download.file(swir_url, destfile = file_path, mode = "wb")
    } else {
      print(paste("Plik SWIR już istnieje:", basename(file_path)))
    }
  } else {
    print(paste("Nie znaleziono kanału SWIR dla kafla:", tile))
  }
}
